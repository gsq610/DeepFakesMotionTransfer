...
class BodyRecoveryFlow(torch.nn.Module):
    ...
    def forward(self, src_img, ref1_img, ref2_img, ref3_img, src_smpl, ref1_smpl, ref2_smpl, ref3_smpl):
        # get smpl information
        src_info = self._hmr.get_details(src_smpl)
        ref1_info = self._hmr.get_details(ref1_smpl)
        ref2_info = self._hmr.get_details(ref2_smpl)
        ref3_info = self._hmr.get_details(ref3_smpl)
        ...
        _, ref1_fim, ref1_wim = self._render.render_fim_wim(ref1_info['cam'], ref1_info['verts'])
        ref1_cond, _ = self._render.encode_fim(ref1_info['cam'], ref1_info['verts'], fim=ref1_fim, transpose=True)
        T1 = self._render.cal_bc_transform(src_f2verts, ref1_fim, ref1_wim)
        syn_img1 = F.grid_sample(src_img, T1)

        _, ref3_fim, ref3_wim = self._render.render_fim_wim(ref3_info['cam'], ref3_info['verts'])
        ref3_cond, _ = self._render.encode_fim(ref3_info['cam'], ref3_info['verts'], fim=ref3_fim, transpose=True)
        T3 = self._render.cal_bc_transform(src_f2verts, ref3_fim, ref3_wim)
        syn_img3 = F.grid_sample(src_img, T3)

        # src input
        input_G_src = torch.cat([src_img * (1 - src_crop_mask), src_cond], dim=1)

        # tsf input
        input_G_tsf1 = torch.cat([syn_img1, ref1_cond], dim=1)
        input_G_tsf3 = torch.cat([syn_img3, ref3_cond], dim=1)
        ...
        if self._opt.bg_both:
            ref1_bg_mask = util.morph(ref1_cond[:, -1:, :, :], ks=15, mode='erode')
            ref3_bg_mask = util.morph(ref3_cond[:, -1:, :, :], ks=15, mode='erode')
            input_G_tsf_bg = torch.cat([ref1_img * ref1_bg_mask, ref3_img * ref3_bg_mask, ref1_bg_mask, ref3_bg_mask], dim=1)
        else:
            input_G_tsf_bg = None

        # masks
        tsf1_crop_mask = util.morph(ref1_cond[:, -1:, :, :], ks=3, mode='erode')
        tsf3_crop_mask = util.morph(ref3_cond[:, -1:, :, :], ks=3, mode='erode')

        head_bbox = self.cal_head_bbox(ref2_info['j2d'])
        body_bbox = self.cal_body_bbox(ref2_info['j2d'])

        return input_G_src_bg, input_G_tsf_bg, input_G_src, input_G_tsf1, input_G_tsf3, \
               T1, T3, src_crop_mask, tsf1_crop_mask, tsf3_crop_mask, head_bbox, body_bbox
...
class Impersonator(BaseModel):
    def __init__(self, opt):
        ...
        self._name = 'Impersonator Temporal Smoothing'
    ...
    def set_input(self, input):
        ...
        with torch.no_grad():
            ...
            tsf1_img = images[:, 1, ...].cuda()
            tsf1_smpl = smpls[:, 1, ...].cuda()
            tsf2_img = images[:, 2, ...].cuda()
            tsf2_smpl = smpls[:, 2, ...].cuda()
            tsf3_img = images[:, 3, ...].cuda()
            tsf3_smpl = smpls[:, 3, ...].cuda()

            input_G_src_bg, input_G_tsf_bg, input_G_src, input_G_tsf1, input_G_tsf3, T1, T3, src_crop_mask, \
                tsf1_crop_mask, tsf3_crop_mask, head_bbox, body_bbox = self._bdr(src_img, tsf1_img, tsf2_img, tsf3_img, src_smpl, tsf1_smpl, tsf2_smpl, tsf3_smpl)
            ...
            self._input_G_tsf1 = input_G_tsf1
            self._input_G_tsf3 = input_G_tsf3
            self._T1 = T1
            self._T3 = T3
    ...
    def forward(self, keep_data_for_visuals=False, return_estimates=False):
        ...
        fake_bg, fake_src_color, fake_src_mask, fake_tsf_color, fake_tsf_mask = \
            self._G.forward(self._input_G_bg, self._input_G_src, self._input_G_tsf1, self._input_G_tsf3, T1=self._T1, T3=self._T3)
