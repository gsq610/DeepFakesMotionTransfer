...
class Imitator(BaseModel):
    ...
    def inference(self, tgt_paths, tgt_smpls=None, cam_strategy='smooth',
        ...
        for t in process_bar:
            tgt_path1, tgt_path2, tgt_path3 = tgt_paths[t]
            tgt_smpl1, tgt_smpl2, tgt_smpl3 = tgt_smpls[t] if tgt_smpls is not None else (None, None, None)

            tsf1_inputs, tsf3_inputs = self.transfer_params(tgt_path1, tgt_path3, tgt_smpl1, tgt_smpl3, cam_strategy, t=t)
            preds = self.forward(tsf1_inputs, tsf3_inputs, self.tsf1_info['T'], self.tsf3_info['T'])

            if visualizer is not None:
                gt = cv_utils.transform_img(self.tsf1_info['image'], image_size=self._opt.image_size, transpose=True)
                visualizer.vis_named_img('pred_' + cam_strategy, preds)
                visualizer.vis_named_img('gt', gt[None, ...], denormalize=False)

                gt = cv_utils.transform_img(self.tsf3_info['image'], image_size=self._opt.image_size, transpose=True)
                visualizer.vis_named_img('pred_' + cam_strategy, preds)
                visualizer.vis_named_img('gt', gt[None, ...], denormalize=False)
            ...
            if output_dir:
                filename = os.path.split(tgt_path2)[-1]

                cv_utils.save_cv2_img(preds, os.path.join(output_dir, 'pred_' + filename), normalize=True)
                cv_utils.save_cv2_img(self.tsf1_info['image'], os.path.join(output_dir, 'gt_' + filename),
    ...
    def swap_smpl(self, src_cam, src_shape, tgt_smpl, cam_strategy='smooth', first_cam=None):
    ...
    def transfer_params_by_smpl(self, tgt_smpl1, tgt_smpl3, cam_strategy='smooth', t=0):
        # get source info
        src_info = self.src_info

        if isinstance(tgt_smpl1, np.ndarray) and isinstance(tgt_smpl3, np.adarray):
            tgt_smpl1 = torch.tensor(tgt_smpl1).float().cuda()[None, ...]
            tgt_smpl3 = torch.tensor(tgt_smpl3).float().cuda()[None, ...]

        if t == 0 and cam_strategy == 'smooth':
            self.first_cam1 = tgt_smpl1[:, 0:3].clone()
            self.first_cam3 = tgt_smpl3[:, 0:3].clone()

        # get transfer smpl
        tsf1_smpl = self.swap_smpl(src_info['cam'], src_info['shape'], tgt_smpl1, cam_strategy=cam_strategy, first_cam=self.first_cam1)
        tsf3_smpl = self.swap_smpl(src_info['cam'], src_info['shape'], tgt_smpl3, cam_strategy=cam_strategy, first_cam=self.first_cam3)
        # transfer process, {'theta', 'cam', 'pose', 'shape', 'verts', 'j2d', 'j3d'}
        tsf1_info = self.hmr.get_details(tsf1_smpl)
        tsf3_info = self.hmr.get_details(tsf3_smpl)

        tsf1_f2verts, tsf1_fim, tsf1_wim = self.render.render_fim_wim(tsf1_info['cam'], tsf1_info['verts'])
        tsf3_f2verts, tsf3_fim, tsf3_wim = self.render.render_fim_wim(tsf3_info['cam'], tsf3_info['verts'])
        # src_f2pts = src_f2verts[:, :, :, 0:2]
        tsf1_info['fim'] = tsf1_fim
        tsf1_info['wim'] = tsf1_wim
        tsf1_info['cond'], _ = self.render.encode_fim(tsf1_info['cam'], tsf1_info['verts'], fim=tsf1_fim, transpose=True)

        tsf3_info['fim'] = tsf3_fim
        tsf3_info['wim'] = tsf3_wim
        tsf3_info['cond'], _ = self.render.encode_fim(tsf3_info['cam'], tsf3_info['verts'], fim=tsf3_fim, transpose=True)
        # tsf_info['sil'] = util.morph((tsf_fim != -1).float(), ks=self._opt.ft_ks, mode='dilate')

        T1 = self.render.cal_bc_transform(src_info['p2verts'], tsf1_fim, tsf1_wim)
        tsf1_img = F.grid_sample(src_info['img'], T1)
        tsf1_inputs = torch.cat([tsf1_img, tsf1_info['cond']], dim=1)

        T3 = self.render.cal_bc_transform(src_info['p2verts'], tsf3_fim, tsf3_wim)
        tsf3_img = F.grid_sample(src_info['img'], T3)
        tsf3_inputs = torch.cat([tsf3_img, tsf3_info['cond']], dim=1)

        # add target image to tsf info
        tsf1_info['tsf_img'] = tsf1_img
        tsf3_info['tsf_img'] = tsf3_img
        tsf1_info['T'] = T1
        tsf3_info['T'] = T3

        self.tsf1_info = tsf1_info
        self.tsf3_info = tsf3_info

        return tsf1_inputs, tsf3_inputs

    def transfer_params(self, tgt_path1, tgt_path3, tgt_smpl1=None, tgt_smpl3=None, cam_strategy='smooth', t=0):
        ori_img1 = cv_utils.read_cv2_img(tgt_path1)
        ori_img3 = cv_utils.read_cv2_img(tgt_path3)
        if tgt_smpl1 is None or tgt_smpl3 is None:
            img_hmr = cv_utils.transform_img(ori_img1, 224, transpose=True) * 2 - 1.0
            img_hmr = torch.tensor(img_hmr, dtype=torch.float32).cuda()[None, ...]
            tgt_smpl1 = self.hmr(img_hmr)

            img_hmr = cv_utils.transform_img(ori_img3, 224, transpose=True) * 2 - 1.0
            img_hmr = torch.tensor(img_hmr, dtype=torch.float32).cuda()[None, ...]
            tgt_smpl3 = self.hmr(img_hmr)
        else:
            if isinstance(tgt_smpl1, np.ndarray):
                tgt_smpl1 = torch.tensor(tgt_smpl1, dtype=torch.float32).cuda()[None, ...]

            if isinstance(tgt_smpl3, np.ndarray):
                tgt_smpl3 = torch.tensor(tgt_smpl3, dtype=torch.float32).cuda()[None, ...]

        tsf1_inputs, tsf3_inputs = self.transfer_params_by_smpl(tgt_smpl1=tgt_smpl1, tgt_smpl3=tgt_smpl3, cam_strategy=cam_strategy, t=t)
        self.tsf1_info['image'] = ori_img1
        self.tsf3_info['image'] = ori_img3
    ...
    def forward(self, tsf1_inputs, tsf3_inputs, T1, T3):
        bg_img = self.src_info['bg']
        src_encoder_outs, src_resnet_outs = self.src_info['feats']

        tsf_color, tsf_mask = self.generator.inference(src_encoder_outs, src_resnet_outs, tsf1_inputs, tsf3_inputs, T1, T3)
    ...
    def post_personalize(self, out_dir, data_loader, visualizer, verbose=True):
        ...
        def set_gen_inputs(sample):
            j2ds1 = sample['j2d1'].cuda()  # (N, 4)
            j2ds3 = sample['j2d3'].cuda()  # (N, 4)
            T1 = sample['T1'].cuda()  # (N, h, w, 2)
            T3 = sample['T3'].cuda()  # (N, h, w, 2)
            T1_cycle = sample['T1_cycle'].cuda()  # (N, h, w, 2)
            T3_cycle = sample['T3_cycle'].cuda()  # (N, h, w, 2)
            src_inputs = sample['src_inputs'].cuda()  # (N, 6, h, w)
            tsf1_inputs = sample['tsf1_inputs'].cuda()  # (N, 6, h, w)
            tsf3_inputs = sample['tsf3_inputs'].cuda()  # (N, 6, h, w)
            src_fim = sample['src_fim'].cuda()
            tsf1_fim = sample['tsf1_fim'].cuda()
            tsf3_fim = sample['tsf3_fim'].cuda()
            init_preds = sample['preds'].cuda()
            images = sample['images']
            images = torch.cat([images[:, 0, ...], images[:, 1, ...], images[:, 3, ...]], dim=0).cuda()  # (2N, 3, h, w)
            pseudo_masks = sample['pseudo_masks']
            pseudo_masks = torch.cat([pseudo_masks[:, 0, ...], pseudo_masks[:, 1, ...]],
                                     dim=0).cuda()  # (2N, 1, h, w)

            return src_fim, tsf1_fim, tsf3_fim, j2ds1, j2ds3, T1, T3, T1_cycle, T3_cycle, \
                   src_inputs, tsf1_inputs, tsf3_inputs, images, init_preds, pseudo_masks

        def set_cycle_inputs(fake_tsf_imgs, src_inputs, tsf1_inputs, tsf3_inputs, T1_cycle, T3_cycle):
            # set cycle src inputs
            cycle_src_inputs = torch.cat([fake_tsf_imgs * tsf1_inputs[:, -1:, ...], tsf1_inputs[:, 3:]], dim=1)

            # set cycle tsf inputs
            cycle_tsf_img1 = F.grid_sample(fake_tsf_imgs, T1_cycle)
            cycle_tsf_img3 = F.grid_sample(fake_tsf_imgs, T3_cycle)
            cycle_tsf1_inputs = torch.cat([cycle_tsf_img1, src_inputs[:, 3:]], dim=1)
            cycle_tsf3_inputs = torch.cat([cycle_tsf_img3, src_inputs[:, 3:]], dim=1)

            return cycle_src_inputs, cycle_tsf1_inputs, cycle_tsf3_inputs
        ...
        def inference(src_inputs, tsf1_inputs, tsf3_inputs, T1, T3, T1_cycle, T3_cycle, src_fim, tsf1_fim, tsf3_fim):
            fake_src_color, fake_src_mask, fake_tsf_color, fake_tsf_mask = \
                self.generator.infer_front(src_inputs, tsf1_inputs, tsf3_inputs, T1=T1, T3=T3)
            ...
            if self._opt.front_warp:
                fake_tsf_imgs = warp(fake_tsf_imgs, tsf1_inputs[:, 0:3], tsf1_fim, fake_tsf_mask)

            cycle_src_inputs, cycle_tsf1_inputs, cycle_tsf3_inputs = set_cycle_inputs(
                fake_tsf_imgs, src_inputs, tsf1_inputs, tsf3_inputs, T1_cycle, T3_cycle)

            cycle_src_color, cycle_src_mask, cycle_tsf_color, cycle_tsf_mask = \
                self.generator.infer_front(cycle_src_inputs, cycle_tsf1_inputs, cycle_tsf3_inputs, T1=T1_cycle, T3=T3_cycle)
        ...
        for epoch in logger:
            for i, sample in enumerate(data_loader):
                ...
                cycle_warp_imgs = F.grid_sample(fake_tsf_imgs, T1_cycle)
                back_head_mask = 1 - self.render.encode_front_fim(tsf1_fim, transpose=True, front_fn=False)
                struct_loss = idt_cri(init_preds, fake_tsf_imgs) + \
                              2 * idt_cri(str_src_imgs * back_head_mask, cycle_warp_imgs * back_head_mask)

                fid_loss = face_cri(src_imgs, cycle_tsf_imgs, kps1=j2ds1[:, 0], kps2=j2ds1[:, 0]) + \
                           face_cri(init_preds, fake_tsf_imgs, kps1=j2ds1[:, 1], kps2=j2ds1[:, 1])
